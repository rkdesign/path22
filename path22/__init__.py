...
def validatePath22BrandName(doc):
    if not doc['value']:
...
def validatePath22WebrootPath(doc):
    if not doc['value']:
...
def validatePath22AlternateWebrootPath(doc):
    if re.match(r'(^,|)girder(,|$)', doc['value']):
...
def validatePath22QuarantineFolder(doc):
...
def restrict_downloads(info):
    ...
    """
    curConfig = config.getConfig().get('Path22', {})
...
class GirderPlugin(plugin.GirderPlugin):
    ...
    def load(self, info):  # noqa
        ...
        ModelImporter.registerModel('aperio', Aperio, 'Path22')
        ModelImporter.registerModel('case', Case, 'Path22')
        ModelImporter.registerModel('cohort', Cohort, 'Path22')
        ModelImporter.registerModel('image', Image, 'Path22')
        ModelImporter.registerModel('pathology', Pathology, 'Path22')
        ModelImporter.registerModel('slide', Slide, 'Path22')
        ...
        huiRoot = WebrootPath22(_template)
        ...
        events.bind('data.process', 'path22.annotations', handlers.process_annotations)
        # Auto-ingest metadta into parent when a file with an identifier
        # ending in 'ItemMetadata' is uploaded (usually .meta files).
        events.bind('data.process', 'path22.metadata', handlers.process_metadata)

        events.bind('model.job.save', 'path22', _saveJob)
        ...
        events.bind('model.setting.save.after', 'path22', updateWebroot)
